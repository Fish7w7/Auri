...
export function WorkCover({ work, compact = false }: { work: Work; compact?: boolean }) {
  ...
  useEffect(() => {
    let active = true
    const load = () => { setCover({ state: 'placeholder', dataUrl: null, source: 'none', cached: false }); void window.lumi.covers.get({ workId: work.id }).then((value) => { if (active) setCover(value) }).catch(() => { if (active) setCover({ state: 'error', dataUrl: null, source: 'none', cached: false }) }) }
    const cleared = () => load()
    load()
    window.addEventListener('lumi:cover-cache-changed', cleared)
    return () => { active = false; window.removeEventListener('lumi:cover-cache-changed', cleared) }
