...
export function CollectionsPage({ collectionId }: { collectionId?: string }) {
  ...
  return <div className="page collections-page">
    ...
    {state === 'ready' && collections.length > 0 && <div className="collections-list">{collections.map((collection) => <article className="collection-row" key={collection.id}><button className="collection-row__main" onClick={() => navigate(`/collections/${collection.id}`)}><strong>{collection.name}</strong><span>{collection.description || 'Sem descrição'}</span></button><div className="collection-row__actions"><Button variant="ghost" onClick={() => openEditor(collection)}>Renomear</Button><Button variant="ghost" onClick={() => setDeleting(collection)}>Excluir</Button></div></article>)}</div>}
