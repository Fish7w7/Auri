...
export function WorkPage({ id }: { id: string }) {
  ...
  return <div className="page work-page">
    <div className="work-page__topbar"><Button variant="ghost" icon="chevron-left" onClick={() => navigate('/library')}>Biblioteca</Button><details className="work-overflow"><summary aria-label="Mais ações">•••</summary><div><button onClick={() => setDialog('edit')}>Editar obra</button>{syncedReference && <button onClick={() => setDialog('metadata')}>Atualizar metadados</button>}<button onClick={() => { setEditingSource(null); setDialog('source') }}>Gerenciar fontes</button><button onClick={() => setDialog('cover')}>Alterar capa</button><button className="is-danger" onClick={() => setTrashOpen(true)}>Mover para Lixeira</button></div></details></div>
    ...
    <div className="work-content">
      <NoteSection title="Onde parei" value={work.lastReadNote ?? ''} placeholder="Registre o ponto exato para retomar." onSave={async (value) => { await window.lumi.works.update({ id: work.id, lastReadNote: value || null }); reload() }} />
...
function Section({ title, action, onAction, children }: { title: string; action?: string; onAction?(): void; children: React.ReactNode }) { return <section className="work-section"><div className="work-section__heading"><h2>{title}</h2>{action && <button onClick={onAction}>+ {action}</button>}</div>{children}</section> }
...
function NoteSection({ title, value, placeholder, onSave }: { title: string; value: string; placeholder: string; onSave(value: string): Promise<void> }) { const [editing, setEditing] = useState(false); const [draft, setDraft] = useState(value); const [busy, setBusy] = useState(false); const { showToast } = useToast(); useEffect(() => setDraft(value), [value]); return <Section title={title} action={editing ? undefined : value ? 'Editar' : 'Adicionar'} onAction={() => setEditing(true)}>{editing ? <div className="note-editor"><textarea autoFocus rows={5} value={draft} onChange={(event) => setDraft(event.target.value)} placeholder={placeholder} /><div><Button onClick={() => { setDraft(value); setEditing(false) }}>Cancelar</Button><Button variant="primary" disabled={busy} onClick={async () => { setBusy(true); try { await onSave(draft.trim()); setEditing(false); showToast({ kind: 'success', message: 'Nota salva.' }) } catch (error) { showToast({ kind: 'error', message: mapDomainError(error) }) } finally { setBusy(false) } }}>Salvar</Button></div></div> : value ? <p className="personal-note">“{value}”</p> : <p className="inline-empty">{placeholder}</p>}</Section> }
