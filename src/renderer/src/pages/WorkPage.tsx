...
export function WorkPage({ id }: { id: string }) {
  ...
  async function updateFavorite() {
    ...
    try { await window.auri.works.update({ id: work.id, favorite }); refreshData() }
  ...
  return <div className="page work-page">
    ...
    <div className="work-content">
      ...
      <Section title="Títulos alternativos" action="Adicionar" onAction={() => setRelation('alias')}><ChipList items={details.aliases.map((alias) => ({ id: alias.id, label: alias.name, hint: alias.kind === 'original' ? 'Original' : 'Alternativo' }))} onRemove={(id) => void removeRelation(() => window.auri.aliases.delete({ aliasId: id }), reload, showToast)} empty="Nenhum título alternativo." /></Section>
      <Section title="Gêneros" action="Adicionar" onAction={() => setRelation('genre')}><ChipList items={details.genres.map((genre) => ({ id: genre.id, label: genre.name }))} onRemove={(genreId) => void removeRelation(() => window.auri.genres.removeFromWork({ workId: work.id, genreId }), reload, showToast)} empty="Nenhum gênero." /></Section>
