...
export function LibraryPage({ initialStatus, initialFavorite, initialSort, collection, onEditCollection, onDeleteCollection }: { initialStatus?: UserStatus; initialFavorite?: boolean; initialSort?: LibrarySort; collection?: Collection; onEditCollection?(): void; onDeleteCollection?(): void }) {
  ...
  const [collectionTotal, setCollectionTotal] = useState(collection?.workCount ?? 0)
  ...
  const load = useCallback(async () => {
    try { setState((current) => current === 'ready' ? current : 'loading'); const [next, total] = await Promise.all([window.auri.library.query(effectiveQuery), collectionId ? window.auri.library.query({ collectionIds: [collectionId] }) : Promise.resolve([])]); setWorks(next); if (collectionId) setCollectionTotal(total.length); dispatchSelection({ type: 'reconcile', workIds: next.map((work) => work.id) }); setState('ready') }
  ...
  return <div className="page library-page">
    ...
    <div className="library-content">
      ...
      {state === 'ready' && works.length === 0 && (search ? <EmptyState title={`Nenhuma obra encontrada para “${search}”.`} description="Tente outro termo ou limpe sua pesquisa." action={<Button onClick={() => setSearch('')}>Limpar pesquisa</Button>} /> : chips.length ? <EmptyState title="Nenhuma obra corresponde aos filtros." description="Remova alguns filtros para ampliar os resultados." action={<Button onClick={() => setQuery({ sort: query.sort })}>Limpar filtros</Button>} /> : collectionId ? <EmptyState title="Esta coleção ainda está vazia." action={<Button variant="primary" icon="plus" onClick={() => setAddWorksOpen(true)}>Adicionar obras</Button>} /> : <EmptyState title="Sua biblioteca ainda está vazia." description="Adicione sua primeira obra para começar a acompanhar suas leituras." action={<Button variant="primary" icon="plus" onClick={openAddWork}>Adicionar obra</Button>} />)}
