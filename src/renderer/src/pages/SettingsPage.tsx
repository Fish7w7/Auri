...
export function SettingsPage() {
  ...
  useEffect(() => { void window.auri.system.getStatus().then(setSystem); loadCache() }, [loadCache])
  useEffect(() => { const refresh = () => loadCache(); window.addEventListener('auri:cover-cache-changed', refresh); return () => window.removeEventListener('auri:cover-cache-changed', refresh) }, [loadCache])
  useEffect(() => { if (section === 'Biblioteca') loadCache() }, [loadCache, section])
  useEffect(() => { if (section === 'Backup') void window.auri.backup.state().then(setBackup).catch((error) => showToast({ kind: 'error', message: error instanceof Error ? error.message : 'Não foi possível carregar os backups.' })) }, [section, settings, showToast])
    ...
    <div className="settings-layout">
      ...
      <section ref={panelRef} className="settings-panel">
        ...
        {section === 'Backup' && <>
          ...
          <SettingsGroup title="Backup automático" description={`Recuperação completa do ${APP_BRAND.name}, incluindo banco, preferências e arquivos permanentes.`}>
            ...
            <SettingRow title="Pasta dos backups" description={<span className={!backup?.directoryAvailable ? 'setting-warning' : ''}>{backup?.directory ?? 'Carregando…'}{backup && !backup.directoryAvailable ? ' · pasta indisponível; será usada a pasta padrão.' : ''}</span>}>
              <div className="setting-inline"><Button disabled={busy} onClick={() => void dataAction(() => window.auri.backup.openFolder(), 'Pasta de backups aberta.')}>Abrir pasta</Button><Button disabled={busy} onClick={() => void dataAction(async () => { const state = await window.auri.backup.chooseDirectory(); if (!state) return false; setBackup(state) }, 'Pasta de backups atualizada.')}>Alterar</Button></div>
            </SettingRow>
          </SettingsGroup>

          <SettingsActionGroup title="Backup" description={`Crie uma cópia agora ou restaure o ${APP_BRAND.name} a partir de um backup completo.`}>
            <Button variant="primary" disabled={busy} onClick={() => void dataAction(async () => { await window.auri.backup.create() }, 'Backup criado com sucesso.')}>Fazer backup agora</Button>
            <Button disabled={busy} onClick={() => void dataAction(async () => { const preview = await window.auri.backup.chooseRestore(); if (!preview) return false; setRestorePreview(preview); return false }, '')}>Restaurar backup</Button>
          </SettingsActionGroup>
          ...
          <SettingsActionGroup title="Portabilidade" description="Exporte dados portáveis ou mescle outra biblioteca JSON com a atual.">
            ...
            <Button disabled={busy} onClick={async () => { try { const preview = await window.auri.transfer.chooseImport(); if (preview) setImportPreview(preview) } catch (error) { showToast({ kind: 'error', message: error instanceof Error ? error.message : 'Arquivo de importação inválido.' }) } }}>Importar JSON</Button>
          </SettingsActionGroup>

          <div className="backup-list"><h3>Backups disponíveis</h3>{backup?.backups.length ? backup.backups.map((item) => <article key={item.path}><div><strong>{backupTypeLabel(item.type)}</strong><p>{new Date(item.createdAt).toLocaleString('pt-BR')} · {formatBytes(item.size)}</p></div><Button variant="ghost" disabled={busy} onClick={() => { if (window.confirm(`Excluir ${item.fileName}?`)) void dataAction(async () => window.auri.backup.delete({ path: item.path }), 'Backup excluído.') }}>Excluir</Button></article>) : <p className="muted-copy">Nenhum backup criado nesta pasta.</p>}</div>
        </>}
...
function AdvancedSettings({ diagnostics, loadError, busy, onBusy, reload, showToast }: {
  ...
  if (!diagnostics) return <><SettingsHeading title="Diagnóstico e manutenção" description={`Informações locais e operações seguras para manter o ${APP_BRAND.name} funcionando.`} />{loadError ? <ErrorState title="Não foi possível carregar o diagnóstico." description="A Biblioteca não foi alterada." onRetry={() => void reload().catch(() => undefined)} /> : <LoadingState />}</>
  ...
  return <>
    <SettingsHeading title="Diagnóstico e manutenção" description={`Informações locais e operações seguras para manter o ${APP_BRAND.name} funcionando.`} />
    <SettingsGroup title="Informações do aplicativo" description="Versões reais desta instalação e dos formatos locais.">
      <div className="diagnostic-card"><dl>
        <div><dt>Aplicativo</dt><dd>{APP_BRAND.name} {status.appVersion}</dd></div>
    ...
    <SettingsGroup title="Pastas importantes" description={`Abra somente os diretórios administrados pelo ${APP_BRAND.name}.`}>
    ...
    <SettingsGroup title="Manutenção segura" description="Estas ações não modificam obras, progresso ou metadados.">
      <SettingRow title="Limpar cache de capas" description="Remove somente arquivos regeneráveis ou baixados. Obras e capas personalizadas são preservadas."><Button disabled={busy} onClick={() => void run(async () => { await window.auri.system.clearCoverCache(); window.dispatchEvent(new Event('auri:cover-cache-changed')) }, 'Cache de capas limpo.', true)}>Limpar cache</Button></SettingRow>
      <SettingRow title="Verificar integridade da biblioteca" description="Executa verificações SQLite somente de leitura; nenhuma correção automática é aplicada."><Button disabled={busy} onClick={() => void run(() => window.auri.system.checkIntegrity(), 'Verificação concluída.', true)}>Verificar integridade</Button></SettingRow>
    ...
    <SettingsActionGroup title="Suporte" description="Gere informações técnicas sem incluir dados pessoais da sua Biblioteca.">
      <Button disabled={busy} onClick={() => void run(() => window.auri.system.copySystemInfo(), 'Informações do sistema copiadas.')}>Copiar informações do sistema</Button>
