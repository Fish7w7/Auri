...
export class AniListProvider implements MetadataProvider {
  ...
  async search(query: string, signal?: AbortSignal) {
    const data = await this.client.query(SEARCH_QUERY, { search: query, perPage: 8 }, signal)
  ...
  async getById(externalId: string, signal?: AbortSignal) {
    const id = Number(externalId)
    if (!Number.isSafeInteger(id) || id <= 0) throw new DomainError('METADATA_NOT_FOUND', 'Obra não encontrada no AniList.')
    const data = await this.client.query(DETAILS_QUERY, { id }, signal)
