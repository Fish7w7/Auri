...
describe('identidade do aplicativo Auri', () => {
  it('usa uma identidade técnica nova e coerente em todo o empacotamento', () => {
    ...
    expect(manifest.build.win.icon).toBe('build/assets/icon.ico')
    expect(manifest.build.nsis).toMatchObject({ shortcutName: 'Auri', installerIcon: 'build/assets/icon.ico', uninstallerIcon: 'build/assets/icon.ico', installerHeaderIcon: 'build/assets/icon.ico' })
