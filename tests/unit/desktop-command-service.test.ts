...
describe('DesktopCommandService', () => {
  it('restaura, mostra e foca a janela existente sem criar outra', () => {
    const send = vi.fn()
    const window = { isDestroyed: () => false, isMinimized: () => true, restore: vi.fn(), show: vi.fn(), focus: vi.fn(), webContents: { isLoadingMainFrame: () => false, send, once: vi.fn() } }
  ...
  it('envia o draft sem criar obra', () => {
    const send = vi.fn()
    const window = { isDestroyed: () => false, isMinimized: () => false, restore: vi.fn(), show: vi.fn(), focus: vi.fn(), webContents: { isLoadingMainFrame: () => false, send, once: vi.fn() } }
    const service = new DesktopCommandService((() => window) as never)
    const draft = { pageUrl: 'https://example.com/obra', title: 'Obra' }
