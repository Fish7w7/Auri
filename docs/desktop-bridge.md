...
Desenvolvimento é ativado por `AURI_NATIVE_HOST_MODE=dev`, usa `%APPDATA%/Auri-Dev` e nunca inicia `npm`, Electron ou outro processo de desenvolvimento. Nesse modo, diagnósticos também podem ser enviados para `stderr`, sem contaminar `stdout`.
