...
### Simular o updater no desenvolvimento

O updater real continua restrito ao aplicativo empacotado. Para testar a tela **Configurações → Atualizações** sem rede, release ou instalador, ative o mock explicitamente no PowerShell:
...
Com o cenário `available`, use **Verificar atualizações**, **Baixar atualização** e **Reiniciar e instalar** na própria interface. A instalação final é apenas registrada como simulada e não fecha o aplicativo.

Os cenários disponíveis são `up-to-date`, `available`, `download`, `ready`, `check-error` e `download-error`. Se a variável de cenário for omitida ou inválida, o Auri usa `available`. Para voltar ao desenvolvimento normal:
